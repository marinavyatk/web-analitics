...
import { theadIncomes } from '@/utils/theads.ts'
import { getRowIncomes } from '@/utils/rows.ts'
...
const rows = computed(() => data.value.map((item) => getRowIncomes(item)))
...
const prepareDiagramData = () => {
  ...
  data.value.forEach((el) => {
    grouped[el.date] = (grouped[el.date] || 0) + el.quantity
  })
...
const fetchData = async () => {
  ...
  const response = await api.getIncomes(queryParams)
...
<template>
  <div class="flex gap-7 items-start mb-7 flex-col lg:flex-row">
    <Card class="flex-grow-2 w-full lg:w-auto">
      <LinearDiagram :series="series" title="Количество продаж" />
    </Card>
